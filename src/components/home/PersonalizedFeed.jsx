    ...
    <section className="mb-8 rounded-2xl bg-white p-6 shadow-md dark:bg-slate-800">
      <div className="mb-5 flex items-center justify-between gap-4">
        <div>
          <h2 className="flex items-center gap-2 text-xl font-bold text-slate-800 dark:text-white">
            <Sparkles className="text-[#6A0032]" size={22} />
            Para ti
          </h2>
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {recomendaciones.map((item) => (
          <Link key={item.id} to={item.path} className="rounded-xl border border-slate-200 p-4 transition hover:border-[#6A0032] hover:bg-slate-50 dark:border-slate-700 dark:hover:bg-slate-700/50">
